    ...
    try {
      ...
      const result = await generateText({
        ...
        messages: [
          {
            role: "system",
            content: MEMORY_OPERATION_PROMPT.replace(
              "{retrieved_old_memory_dict}",
              oldMemoryDict
      ...
      let memoryArray = parsedContent.memory || parsedContent;
      if (!Array.isArray(memoryArray)) {
        throw new Error("Invalid response format");
      }

      // Map the numeric IDs back to UUIDs and format the response
      const results = memoryArray.map((item: any) => {
        // For ADD operations, we don't need an ID (it will be generated by Memory class)
        // For UPDATE/DELETE operations, we need to map the numeric ID to the actual UUID
        const actualId =
          item.event === "ADD" ? crypto.randomUUID() : idMapping.get(item.id);

        return {
          id: actualId || crypto.randomUUID(), // Ensure we always have an ID
          memory: item.text,
          event: item.event as "ADD" | "UPDATE" | "DELETE" | "NONE",
