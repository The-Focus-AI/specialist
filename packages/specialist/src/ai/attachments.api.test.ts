...
(runApiTests ? describe : describe.skip)("Attachment API Tests", () => {
  ...
  beforeAll(async () => {
    ...
    if (!hasOpenAIKey) {
      console.error(
        "OPENAI_API_KEY not available - some tests will be skipped"
      );
    }
  });
  ...
  it("should analyze image content with Anthropic API", async () => {
    ...
    const hasAnthropicKey = await requiresEnv(
      "ANTHROPIC_API_KEY",
      "op://Development/Claude API/notesPlain"
    );
    if (!hasAnthropicKey) {
      console.log(
        "ANTHROPIC_API_KEY not available - skipping Anthropic image test"
      );
      return;
    }
